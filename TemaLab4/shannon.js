var treeData = {
  ...
  function btn_Conversie() {
   var txt_box=document.getElementById("fname").value;
    var vect = [];
    
    Array.from(txt_box).forEach((e, i) => {
        if (vect[e]) {
            vect[e] = vect[e] + 1;
        } else
            vect[e] = 1;
    ...
    var keys = Object.keys(vect).sort().reverse().sort((a, b) => {
        return vect[a] - vect[b];
    }).reverse();
    ...
    var treeData = constShannon(sorted, 0, keys.length, "O", "A", 0);
    
    contArbore(treeData);
    }
    
    function constShannon(sorted, l, r, flag, name, level) {
        var node = {};
        ...
        var index;
        for (var i = l; i < r; i++) {
            if (sum2 + sorted[keys[i]] < sum/2) {
                sum2 += sorted[keys[i]];
            } else {
                if (sum/2 - sum2 < sum2 + sorted[keys[i]] - sum/2) {
                    index = i;
                    break;
                }
                index = i + 1;
        ...
        node["children"] =
            [constShannon(sorted, l, index, "L", node["name"], level + 1),
            constShannon(sorted, index, r, "R", node["name"], level + 1)];
    
        return node;
    }
    
    function contArbore(treeData) {
